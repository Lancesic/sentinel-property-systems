...
export function PageHero({ eyebrow, title, copy, cta = "Schedule a Property Systems Consultation" }: { eyebrow: string; title: string; copy: string; cta?: string }) {
...
export function CtaSection() {
  return <section className="section-pad bg-steel text-white"><div className="container-tight"><div className="grid items-center gap-8 rounded-md border border-white/10 bg-ink p-8 shadow-glow lg:grid-cols-[1fr_auto] lg:p-10"><div className="flex gap-5"><div className="hidden h-14 w-14 flex-none place-items-center rounded-md bg-signal/15 text-signal sm:grid"><Radar className="h-7 w-7" /></div><div><h2 className="font-display text-3xl font-semibold tracking-normal">Stop buying systems nobody maintains.</h2><p className="mt-3 max-w-3xl text-lg leading-8 text-slate-300">Get a property visibility system that is installed cleanly, documented clearly, and maintained properly from day one.</p></div></div><div className="flex flex-col gap-3 sm:flex-row lg:flex-col xl:flex-row"><ButtonLink href="/contact" variant="primary">Schedule a Consultation <ArrowRight className="h-4 w-4" /></ButtonLink><ButtonLink href="/maintenance-plans" variant="secondary">Explore Maintenance Plans</ButtonLink></div></div></div></section>;
}

...
export function ButtonLink({ href, children, variant = "primary", className = "" }: { href: string; children: React.ReactNode; variant?: "primary" | "secondary" | "dark" | "ghost"; className?: string }) {
  const variants = {
    primary: "bg-signal text-ink shadow-glow hover:bg-white hover:text-ink border border-signal",
    secondary: "border border-white/20 bg-white/10 text-white hover:border-white/40 hover:bg-white/15",
    dark: "border border-ink bg-ink text-white hover:bg-steel",
    ghost: "border border-slate-300 bg-white text-ink hover:border-ink hover:bg-frost"
  };
